...
class FactorEvaluationService:
    ...
    async def _saved_factor_board_rows(self, query: BoardQuery) -> list[BoardRow]:
        ...
        from app.services.factor_value_store import list_factor_definitions
        builtins = list_factor_definitions()
        if query.categories:
    ...
    def _load_from_factor_store(
        ...
        try:
            ...
            if not store.exists():
                return None
            # 通用名 → 物理 smallcap_* feature_name 映射
            _NAME_TO_FEATURE = {
                "market_cap": "smallcap_market_cap",
                "market_cap_rank": "smallcap_market_cap_rank",
                "is_st": "smallcap_is_st",
                "is_paused": "smallcap_is_paused",
                "is_limit_up": "smallcap_is_limit_up",
                "is_limit_down": "smallcap_is_limit_down",
                "yesterday_limit_up": "smallcap_yesterday_limit_up",
                "v4gv": "smallcap_v4gv",
                "v4gv_signal": "smallcap_v4gv21",
                "macd_positive": "smallcap_macd_signal",
                "indicator_buy_signal": "smallcap_indicator_buy_signal",
                "v4gv_dead_cross": "smallcap_v4gv_dead_cross",
                "rolling_max_volume": "max_volume_nd",
            }
            resolved = _NAME_TO_FEATURE.get(name, name)
            df = store.load(
                factor_names=[resolved],
                start_date=start_date,
                end_date=end_date,
                symbols=symbols,
            )
