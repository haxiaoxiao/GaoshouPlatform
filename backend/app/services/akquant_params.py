...
class AkquantParamsUnavailable(RuntimeError):
...
def get_strategy_param_schema(strategy_code: str) -> dict[str, Any]:
    ...
    if not AKQUANT_AVAILABLE:
        raise AkquantParamsUnavailable("akquant is not installed")
    ...
    if not hasattr(aq, "get_strategy_param_schema"):
        raise AkquantParamsUnavailable("akquant.get_strategy_param_schema is unavailable")
...
def validate_strategy_params(
    ...
    if not hasattr(aq, "validate_strategy_params"):
        raise AkquantParamsUnavailable("akquant.validate_strategy_params is unavailable")
