...
async def get_system_status():
    ...
    if settings.market_data_backend == "parquet":
        try:
            ...
            minute = store.coverage([], date(2000, 1, 1), date.today(), dataset="klines_minute_timer")
            parquet_info = {
                "klines_daily_rows": daily.get("total_rows", 0),
                "klines_daily_symbols": len(daily.get("symbols_covered", [])),
                "klines_minute_timer_rows": minute.get("total_rows", 0),
                "klines_minute_timer_symbols": len(minute.get("symbols_covered", [])),
            }
        except Exception:
