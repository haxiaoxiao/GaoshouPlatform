...
chcp 65001 >nul
title GaoshouPlatform AI Native Launcher

for %%I in ("%~dp0..\..") do set "SCRIPT_ROOT=%%~fI"
if defined GAOSHOU_ROOT (set "ROOT=%GAOSHOU_ROOT%") else (set "ROOT=%SCRIPT_ROOT%")
set "BACKEND_DIR=%ROOT%\backend"
set "FRONTEND_DIR=%ROOT%\frontend"
if defined GAOSHOU_AI_ENV_FILE (
  set "ENV_FILE=%GAOSHOU_AI_ENV_FILE%"
) else if defined GAOSHOU_ENV_FILE (
  set "ENV_FILE=%GAOSHOU_ENV_FILE%"
) else (
  set "ENV_FILE=%ROOT%\.env.local"
)
if defined GAOSHOU_AI_PYTHON (
  set "PYTHON=%GAOSHOU_AI_PYTHON%"
) else if defined GAOSHOU_PYTHON (
  set "PYTHON=%GAOSHOU_PYTHON%"
) else (
  set "PYTHON=%BACKEND_DIR%\.venv\Scripts\python.exe"
)

if defined GAOSHOU_AI_BACKEND_HOST (
  set "BACKEND_HOST=%GAOSHOU_AI_BACKEND_HOST%"
) else if defined GAOSHOU_BACKEND_HOST (
  set "BACKEND_HOST=%GAOSHOU_BACKEND_HOST%"
) else (
  set "BACKEND_HOST=127.0.0.1"
)
if defined GAOSHOU_AI_BACKEND_PORT (
  set "BACKEND_PORT=%GAOSHOU_AI_BACKEND_PORT%"
) else if defined GAOSHOU_BACKEND_PORT (
  set "BACKEND_PORT=%GAOSHOU_BACKEND_PORT%"
) else (
  set "BACKEND_PORT=18880"
)
if defined GAOSHOU_AI_SYNC_HOST (
  set "SYNC_HOST=%GAOSHOU_AI_SYNC_HOST%"
) else if defined GAOSHOU_SYNC_HOST (
  set "SYNC_HOST=%GAOSHOU_SYNC_HOST%"
) else (
  set "SYNC_HOST=127.0.0.1"
)
if defined GAOSHOU_AI_SYNC_PORT (
  set "SYNC_PORT=%GAOSHOU_AI_SYNC_PORT%"
) else if defined GAOSHOU_SYNC_PORT (
  set "SYNC_PORT=%GAOSHOU_SYNC_PORT%"
) else (
  set "SYNC_PORT=18890"
)
if defined GAOSHOU_AI_FRONTEND_HOST (
  set "FRONTEND_HOST=%GAOSHOU_AI_FRONTEND_HOST%"
) else if defined GAOSHOU_FRONTEND_HOST (
  set "FRONTEND_HOST=%GAOSHOU_FRONTEND_HOST%"
) else (
  set "FRONTEND_HOST=127.0.0.1"
)
if defined GAOSHOU_AI_FRONTEND_PORT (
  set "FRONTEND_PORT=%GAOSHOU_AI_FRONTEND_PORT%"
) else if defined GAOSHOU_FRONTEND_PORT (
  set "FRONTEND_PORT=%GAOSHOU_FRONTEND_PORT%"
) else (
  set "FRONTEND_PORT=13580"
)
if defined GAOSHOU_AI_ENV_LABEL (
  set "APP_ENV_LABEL=%GAOSHOU_AI_ENV_LABEL%"
) else if defined VITE_APP_ENV_LABEL (
  set "APP_ENV_LABEL=%VITE_APP_ENV_LABEL%"
) else (
  set "APP_ENV_LABEL=V2"
)
...
set "TUSHARE_TOKEN="
set "TS_TOKEN="
set "AI_MODEL=deepseek/deepseek-chat"
set "AI_API_KEY_ENV=DEEPSEEK_API_KEY"
...
if exist "%ENV_FILE%" (
  for /f "usebackq tokens=1,* delims==" %%a in ("%ENV_FILE%") do (
    ...
    if /i "!K!"=="AI_MODEL" set "AI_MODEL=!V!"
    if /i "!K!"=="AI_API_KEY_ENV" set "AI_API_KEY_ENV=!V!"
    ...
    if /i "!K!"=="GAOSHOU_AI_BACKEND_PORT" if not defined GAOSHOU_AI_BACKEND_PORT set "BACKEND_PORT=!V!"
    if /i "!K!"=="AI_BACKEND_PORT" if not defined GAOSHOU_AI_BACKEND_PORT set "BACKEND_PORT=!V!"
    if /i "!K!"=="GAOSHOU_AI_SYNC_PORT" if not defined GAOSHOU_AI_SYNC_PORT set "SYNC_PORT=!V!"
    if /i "!K!"=="AI_SYNC_PORT" if not defined GAOSHOU_AI_SYNC_PORT set "SYNC_PORT=!V!"
    if /i "!K!"=="GAOSHOU_AI_FRONTEND_PORT" if not defined GAOSHOU_AI_FRONTEND_PORT set "FRONTEND_PORT=!V!"
    if /i "!K!"=="AI_FRONTEND_PORT" if not defined GAOSHOU_AI_FRONTEND_PORT set "FRONTEND_PORT=!V!"
    if /i "!K!"=="GAOSHOU_AI_ENV_LABEL" if not defined GAOSHOU_AI_ENV_LABEL set "APP_ENV_LABEL=!V!"
    if /i "!K!"=="AI_ENV_LABEL" if not defined GAOSHOU_AI_ENV_LABEL set "APP_ENV_LABEL=!V!"
  )
)
...
set "AI_STATUS_URL=http://%BACKEND_HOST%:%BACKEND_PORT%/api/ai/status"
...
set "VITE_APP_ENV_LABEL=%APP_ENV_LABEL%"
set "QMT_ACCOUNT_STATUS=not configured"
if defined QMT_ACCOUNT_ID set "QMT_ACCOUNT_STATUS=configured"
set "AI_KEY_STATUS=not configured"
if defined AI_API_KEY_ENV (
  call set "AI_KEY_VALUE=%%%AI_API_KEY_ENV%%%"
  if defined AI_KEY_VALUE set "AI_KEY_STATUS=configured via %AI_API_KEY_ENV%"
)

echo ========================================
echo   GaoshouPlatform AI Native Startup
echo ========================================
echo Root:       %ROOT%
...
echo AI status:  %AI_STATUS_URL%
echo Data mode:  %MARKET_DATA_BACKEND%  storage=Parquet/DuckDB
echo AI model:   %AI_MODEL%  key=%AI_KEY_STATUS%
...
echo [1/8] Stopping stale AI Native processes on configured ports...
...
echo [4/8] Starting sync service on %SYNC_HOST%:%SYNC_PORT%...
...
echo [5/8] Starting backend on %BACKEND_HOST%:%BACKEND_PORT%...
...
echo [6/8] Checking AI Native API...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$url='%AI_STATUS_URL%'; try { Invoke-WebRequest -Uri $url -UseBasicParsing -TimeoutSec 5 | Out-Null } catch { exit 1 }"
if errorlevel 1 (
  echo       ERROR: AI Native API status check failed: %AI_STATUS_URL%
  if "%NO_PAUSE%"=="0" pause
  exit /b 1
)
echo       OK
...
echo [8/8] Starting AI Native frontend on %FRONTEND_HOST%:%FRONTEND_PORT%...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c','set VITE_API_PROXY_TARGET=http://%BACKEND_HOST%:%BACKEND_PORT%&& set VITE_APP_ENV_LABEL=%APP_ENV_LABEL%&& npm run dev -- --host %FRONTEND_HOST% --port %FRONTEND_PORT% --strictPort') -WorkingDirectory '%FRONTEND_DIR%' -WindowStyle Hidden"
...
echo.
echo ========================================
echo   AI Native startup complete
echo ========================================
echo Frontend:       %FRONTEND_URL%
echo Copilot:        %FRONTEND_URL%  topbar star button
echo Backend docs:   http://%BACKEND_HOST%:%BACKEND_PORT%/docs
echo AI status:      %AI_STATUS_URL%
echo AI tools:       http://%BACKEND_HOST%:%BACKEND_PORT%/api/ai/tools
echo Sync health:    %SYNC_URL%
echo Stop script:    %ROOT%\tools\ai-native-startup\stop-gaoshouplatform-ai-native.bat
echo.
if "%NO_PAUSE%"=="0" pause
exit /b 0
