...
市场雷达依赖的日终数据不由页面“立即刷新”下载。若页面显示涨跌停、连板或两融来源过期，请按以下顺序操作：

1. 打开 /data/sync，在概览页选择“市场雷达数据”预设，或在目录中单独加入“涨跌停明细”“连板梯队”“两融余额”。
2. 在队列中选择需要的起止日期和失败策略，确认 Relay 凭据已配置后执行同步。三项数据会写入 tushare_limit_list_d、tushare_limit_step 和 tushare_margin Parquet 数据集。
3. 同步完成后返回 /market-radar，点击“立即刷新”重算日终快照；该按钮只负责读取已落库数据和重算，不替代同步。

同步面板的覆盖度和最新日期以 Parquet 实际落库结果为准；若 Relay 请求失败，先查看同步队列的失败详情，再重试对应数据集。
