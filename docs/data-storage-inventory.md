...
本文件记录平台里容易混淆的历史数据集，避免把“派生缓存”误判成无用冗余。审计时间：2026-06-10；文档复核：2026-07-17。

注意：文件数和大小是历史本机快照，不是实时容量或代码层面的硬约束。清理前以当前公共数据目录 `E:\Projects\data\BaiduSyncdisk\parquet` 和 SQLite 实际状态为准。

实时盘点可使用 `Get-ChildItem E:\Projects\data\BaiduSyncdisk\parquet -Recurse -File` 汇总文件数和大小；表行数通过只读 SQLite/DuckDB 查询按需核对，不要把本页快照复制成运行判断。
