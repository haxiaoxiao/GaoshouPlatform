...
下一轮不会继续堆叠常见技术指标。研究优先级是固定扩展到 24 股，跨股票复现方向位移候选，再独立验证严格因果的量价交互与个股/指数共跳；单位成交额价格冲击只用于流动性和容量分层。v4 必须从 2026-07-20 之后的新交易日累计 120 日，现有历史结果不得再次标记为未见样本，也不会自动修改页面或模拟盘参数。
...
研究产物为 `.runtime/intraday-t-v2-research/2024-07-19_2026-07-19/` 和 `.runtime/intraday-t-v3-research/2024-07-19_2026-03-13/` 下的 `research.json`、`runs.csv`。复跑命令见根目录 `README.md`，运行器只读行情与 SQLite 涨跌停价，不会写交易订单。
