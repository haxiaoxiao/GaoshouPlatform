...
- 数据链路：华泰 miniQMT/xtquant 为主数据源，SQLite 存元数据，Parquet/DuckDB 为行情和因子数据层。
- 回测引擎：支持内置事件驱动引擎和 AKQuant 引擎，AKQuant 已接入能力探测、事件驱动回测、Grid Search、Walk-forward Validation、参数 schema/校验。
- 策略数据模式：支持 `daily`、`minute`、`minute_timer`。只需要固定盘中时点的策略应优先用 `minute_timer`，从 Parquet 读取稀疏分钟数据。
...
│   │   ├── core/
│   │   │   └── config.py        # 配置（DB路径、Parquet路径、dev数据模式等）
│   │   ├── api/
│   │   │   ├── router.py        # 路由汇总 /api/system /api/data /api/explorer ...
│   │   │   ├── data.py          # 股票/K线/同步代理/自选股 API
│   │   │   ├── data_explorer.py # Parquet/DuckDB 数据浏览器 API
...
│   │   │   ├── DataExplorer.vue # ⭐ Parquet/DuckDB 数据浏览器
...
| `kline_daily` | 日 K 线 | `download_history_data` + `get_market_data_ex` | Parquet |
| `kline_minute` | 分钟 K 线 | `download_history_data` + `get_market_data_ex` | Parquet |
...
抽取 10:00 / 10:30 / 14:30 / 14:50 等 timer 点
        ↓
Parquet klines_minute_timer / klines_minute
        ↓
...
6. 若缺分钟数据，先用 miniQMT 主动下载，再同步所需 timer 点到 Parquet；若已有完整分钟 Parquet，则直接读取本地数据集。
7. 回测阶段只读 Parquet，避免策略运行时反复读取 QMT。
...
`clean_local_cache()` 只清理以下目录（数据已落地到本地存储后）：
...
### SQLite（默认 `E:\Projects\data\BaiduSyncdisk\gaoshou.db`）

路径通过 `backend/app/core/config.py` 自动解析为绝对路径，默认优先使用公共同步目录 `E:\Projects\data\BaiduSyncdisk`；可用 `GAOSHOU_DATA_DIR`、`DATABASE_URL`、`PARQUET_DATA_DIR` 覆盖。
